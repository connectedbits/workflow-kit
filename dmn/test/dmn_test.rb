...
module DMN
  ...
  describe :evaluate do
    ...
    it "should eval slightly more complex expressions" do
      _(DMN.evaluate('if person.age >= 18 then "adult" else "minor"', variables: { person: { name: "Eric", age: 59 } })).must_equal "adult"
    end
    ...
    it "should eval user defined functions" do
      _(DMN.evaluate('reverse_inline("Hello World!")', variables: { "reverse_inline": ->(s) { s.reverse } })).must_equal "!dlroW olleH"
    end
  ...
  describe :test do
    ...
    it "should match input entry '\"A\"' to the string value \"A\"" do
      _(DMN.test("A", '"A"')).must_equal true
      _(DMN.test("B", '"A"')).must_equal false
    end

    it "should match input entry: '\"A\"', '\"B\"' to the string value \"A\" or \"B\"" do
      _(DMN.test("B", '"A", "B"')).must_equal true
      _(DMN.test("A", '"A", "B"')).must_equal true
      _(DMN.test("C", '"A", "B"')).must_equal false
    end
    ...
    it "should match input entry nil to anything" do
      _(DMN.test(33, nil)).must_equal true
    end
    ...
    it "should match input entry 'date(\"1963-12-23\")' to the date value 1963-12-23" do
      _(DMN.test(Date.new(1963, 12, 23), 'date("1963-12-23")')).must_equal true
      _(DMN.test(Date.new(1963, 12, 24), 'date("1963-12-23")')).must_equal false
    end
    ...
    it "should match input entry 'date(begin) + duration(d)' to the date that results by adding the given duration to the given date" do
      _(DMN.test(Date.new(1963, 12, 26), 'date("1963-12-23") + duration("P3D")')).must_equal true
    end
    ...
    it "should match input entry '< date(begin) + duration(d)' to any date before the date that results by adding the given duration to the given date" do
      _(DMN.test(Date.new(1963, 12, 22), '< date("1963-12-23") + duration("P3D")')).must_equal true
    end
  end
